# Fine-tuning script for Mamba model with PANDA dataset using LoRA and GPU optimization
...
WANDB_PROJECT = "mamba1-panda-finetuning"
WANDB_NAME = "mamba1-2.8b-lora-panda-debiasing"
...
def compute_metrics(eval_preds):
    ...
    # Calculate perplexity (optional)
    # This is computationally expensive, so we'll just report accuracy for now
...
def main():
    ...
    # Load tokenizer explicitly with the correct class
    logger.info(f"Loading tokenizer from {TOKENIZER_NAME}")
    try:
        tokenizer = GPTNeoXTokenizerFast.from_pretrained(TOKENIZER_NAME)
        logger.info("Successfully loaded GPTNeoXTokenizerFast")
    except Exception as e:
        logger.error(f"Error loading GPTNeoXTokenizerFast: {e}")
    ...
    # Load the model with transformers (with trust_remote_code)
    logger.info(f"Loading model from {MODEL_NAME}")
    load_in_4bit = USE_4BIT
    load_in_8bit = USE_8BIT and not USE_4BIT  # Only use 8-bit if 4-bit is disabled

    # Model loading options with trust_remote_code=True
    if load_in_4bit:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            trust_remote_code=True,  # Critical for Mamba models
        )
    ...
    # Configure LoRA for Mamba - inspect model architecture to find correct module names
    logger.info("Inspecting model architecture to determine target modules for LoRA")

    # Attempt to find module names automatically by inspecting model architecture
    import re

    # Get model parameter names
    param_names = [name for name, _ in model.named_parameters()]
    param_names_str = str(param_names)
    logger.info(f"Model parameter list: {param_names_str[:500]}...")  # Log a sample

    # Patterns to look for in parameter names
    pattern_candidates = [
        r"\.([^.]+)\.(weight|bias)$",  # General pattern for layer names
        r"backbone\.layers\.\d+\.([^.]+)\.(weight|bias)$",  # For transformers Mamba structure
    ]
    ...
    for pattern in pattern_candidates:
        for param_name in param_names:
            ...
            if match:
                ...
                if not any(
                    x in module_name for x in ["norm", "layer_norm", "ln"]
                ):  # Exclude norm layers
    ...
    if not target_modules:
        logger.warning(
            "Could not detect target modules automatically, using default list"
        )
        target_modules = ["in_proj", "out_proj", "x_proj", "dt_proj", "A_log"]
    ...
    try:
        ...
        logger.error(f"Error in PEFT configuration: {e}")
        # Fallback to a very basic set of target modules
        logger.info("Trying fallback LoRA configuration")
        peft_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM,
            inference_mode=False,
            r=LORA_RANK,
            lora_alpha=LORA_ALPHA,
            lora_dropout=LORA_DROPOUT,
            target_modules=["query_key_value"],  # Very basic fallback
        )
        model = get_peft_model(model, peft_config)
