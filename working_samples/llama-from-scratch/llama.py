...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # todo
        layer_normalized = self.attention_norm(x)
        self_attentioned = self.attention.forward(layer_normalized)
        h = x + self_attentioned  # residual connection
        layer_normalization_on_h = self.ffn_norm(h)
        ffn_on_layer_norm = self.feed_forward.forward(layer_normalization_on_h)
        return h + ffn_on_layer_norm
...
class Llama(LlamaPreTrainedModel):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            # Commented out original cropping
            ...
            if temperature == 0.0:
                ...
                """
                # softmax to get probabilities and does normalization
                # section 3.3 from https://arxiv.org/pdf/1904.09751.pdf
                probs = torch.softmax(logits / temperature, dim=-1)
                # we can use top_k here if provided
                if top_k:
                    top_k = min(top_k, logits.size(-1))  # safety check
                    idx_next = torch.topk(probs, top_k)
                else:
                    # sample by multinomial
                    idx_next = torch.multinomial(probs.squeeze(0), 1).unsqueeze(0)
